...
const BusinessEvent = () => {
    ...
    const { data: userEventList, refetch } = useQuery(
        ...
        async ({ signal }) => {
            return await axiosPrivate
                .get(USER_API_URL.userEventList + "?userId=" + state?.userId, { signal })
                .then((res) => res.data);
        },
    ...
    useEffect(() => {
        var businessEvent = [];
        userEventList?.map((data) => {
            if (data.businessId) {
                businessEvent.push(data)
            }
        })
        setBusinessEvents(businessEvent)
        setTimeout(() => {
            setLoding(false);
        }, 1800);
    }, [userEventList])
        ...
        <>
            {loding ? <LoadingScreen /> : <>
                <div className="dashboard_header mb-4">
                    <h4 className="app_text_20_semibold mb-0 d-flex align-items-center">business events</h4>
                    ...
                    <TableContainer>
                        <Table>
                            ...
                            {businessEvents?.length > 0 ?
                                <TableBody>
                                    {businessEvents.length != 0 && businessEvents?.map((row) => {
