...
export default function TraceRequest() {
   ...
   let csvReport = {
      data: CsvData,
      headers: headers,
      filename: 'beemz-trace-request.csv'
   };

   async function getTraceRequest(page, filterName) {
      const response = await axiosPrivate.get(TRACE_API_URL.getTraceRequest, { params: { page: page + 1, search: filterName, limit: rowsPerPage } })
      return response.data[0];
   }

   const { isLoading, data: traceRequestList, refetch } = useQuery(['traceRequestList', filterName], () => getTraceRequest(page, filterName), { keepPreviousData: true, })
   useEffect(() => {
      if (traceRequestList) {
         setTableData(traceRequestList.data);
         setRowsPerPage(10);
      }
      setTimeout(() => {
         setLoding(false);
   ...
   const handleFilterByName = (event) => {
      ...
      getTraceRequest(page, filterName);
   };
   ...
   const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - traceRequestList?.data?.length) : 0;
   const filteredUsers = tableData;
   const isUserNotFound = filteredUsers?.length === 0;
   if (isLoading) return <LoadingScreen />

   async function updateTraceRequest(status, id) {
      const response = await axiosPrivate.put(TRACE_API_URL.updatePandingRequest + id, { status: status })
         ...
         {loding ? <LoadingScreen /> : <>
            <div className="dashboard_header mb-4">
               ...
               <Stack direction={"row"} className="d-flex align-items-center" spacing={2}>
                  ...
                  {permissionsData?.trace_request?.substring(1, 2) == "1"
                     ?
            ...
            <Card>
               <TableContainer>
                  <Table>
                     <UserListHead
                        ...
                        rowCount={traceRequestList?.data?.length}
               ...
               <TablePagination
                  ...
                  count={traceRequestList?.metadata?.length != 0 ? traceRequestList?.metadata[0]?.total : 0}
