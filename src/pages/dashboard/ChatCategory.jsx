...
export default function ChatCategory() {
  ...
  let csvReport = {
    data: CsvData,
    headers: headers,
    filename: 'beemz-chat-categories.csv'
  };
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm, setFieldError, setSubmitting }) => {
      ...
      if (parentId != "") {
        ...
        enqueueSnackbar("parentId is required", {
          variant: "error",
  ...
  const { mutateAsync: categoryAdd } = useMutation(
    async (data) => {
      return await axiosPrivate.post(CATEGORY_API_URL.addChatCategory, JSON.stringify(data))
    },
    {
      onSuccess: ({ data }) => {
        refetch();
  ...
  async function fetchSeeker(page, filterName) {
    const response = await axiosPrivate.get(USER_API_URL.chatCategoryList, { params: { type: '1', page: page + 1, search: filterName, limit: rowsPerPage } })
  ...
  async function CategorySelect() {
    const response = await axiosPrivate.get(USER_API_URL.selectChatCategoryList)
  ...
  const deleteCategory = async () => {
    if (type == "sub") {
      const response = await axiosPrivate.delete(USER_API_URL.deleteChatCategory + deleteId)
        ...
        <div className="dashboard_header mb-4">
          <Paper className='dashboard_searchbox col-lg-4'>
            ...
            <InputBase
              fullWidth
              sx={{ flex: 1 }}
              placeholder="search chat category..."
              onChange={(e) => handleFilterByName(e)}
            />
          </Paper>
          <Stack direction={"row"} className="d-flex align-items-center" spacing={2}>
            {permissionsData?.chat_category?.substring(1, 2) == "1"
            ...
            {/* csv */}
            {permissionsData?.chat_category?.substring(1, 2) == "1" ?
              <AppTooltip title="export chat category" placement="bottom"><Button className="dashboard_light_bg_icon_btn" ><CSVLink {...csvReport}><FileUploadOutlinedIcon style={{ fontSize: 18, color: "#6200ee" }} /></CSVLink></Button></AppTooltip>
              : ''}
          </Stack>
        </div>
        <Dialog
              ...
              {filteredUsers.length > 0 ?
                <TableBody>
                  {filteredUsers.map((row, key) => {
                    const { _id, name, user } = row;
                    const createdAt = Date.now(row.createdAt);
                          ...
                          <Stack sx={{ flexWrap: 'wrap' }} direction="row" spacing={1}>
                            {row.child.length > 0 ? row?.child?.map((item, i) => (
                              <Chip style={{ marginBottom: '10px' }} label={item.name}
                                key={i}
                                onDelete={() =>
                                  permissionsData?.category?.substring(2, 3) == "1" ? handleClickOpen(item._id, "sub") : ''
                                } />
                            )) : "-"}
                          </Stack>
                        </TableCell>
                  ...
                  <div className="row">
                    <div className='col-12'>
                      <div className='user_edit_header mb-4'>
                        <h4 className="app_text_14_semibold mb-0"> chat category information</h4>
                      </div>
                    </div>
