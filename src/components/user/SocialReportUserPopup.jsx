...
const SocialReportUserPopup = (props) => {
    ...
    const { data: newLinkRequest, refetch: linkedRequestList } = useQuery("newLinkRequest", async ({ signal }) => {
        if (newLinkId) {
            return await axiosPrivate.get(SOCIAL_API_URL.reportPostUser + newLinkId, { signal }).then(res => res.data)
        }
    ...
    return (
        <Dialog onClose={handleClose} open={open} className="linking_popup_wrapper">
            <DialogTitle className='px-0 pt-0 position-relative'>
                <span className='app_text_20_500'>post report user list</span>
                {onClose ? (
                    <IconButton
            ...
            {newLinkRequest && newLinkRequest.length > 0 ?
                <>
                    ...
                    <h4 className='app_text_gray app_text_12_fw500 py-3 ps-1'>list of users who are post report</h4>
                ...
                :
                <span className='app_text_20_semibold'>no post report user</span>
            }
        </Dialog>
    );
}

SocialReportUserPopup.propTypes = {
    onClose: PropTypes.func.isRequired,
    open: PropTypes.bool.isRequired,
};

export default SocialReportUserPopup